...
const Chart = ({token,productId}) => {
 ...
 useEffect(()=>{
   const saleData = async()=>{
    try{
     const res =activeButton == "products" ? productId ? await axios.get(`http://localhost:3000/api/sales/${productId}`,{headers:{token:token}}) : await axios.get('http://localhost:3000/api/sales',{headers:{token:token}}) : await axios.get('http://localhost:3000/api/user/stats/',{headers:{token:token}})
     setData([])
