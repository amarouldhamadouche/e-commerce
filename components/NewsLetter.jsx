...
const NewsLetter = ({token}) => {
  ...
  const sendMessage = async()=>{
    ...
    try{
     const res = typeof(window)!=="undefined" &&  await axios.post(`${window.location.origin}/api/contact`,{userName:currentUser?.username,text:message},{headers:{token:token}})
     setSuccess(true)
     setMessage('')
  ...
  return (
    <div className={styles.Container}>
     ...
     <div className={styles.InputContainer}>
       <input placeholder='Send us a message' value={message} onChange={(e)=>setMessage(e.target.value)} className={styles.Input} type='email'/>
